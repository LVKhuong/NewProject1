...
class TrangChuController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('thuonghieu')) {
            $dataSanPham = sanpham::where('id_thuonghieu', $request->input('thuonghieu'))->paginate(12);
            $dataSanPham->appends($request->all());
        }else if($request->has('sale')){
            $dataSanPham = sanpham::where('sale','>',0)->paginate(12);
            $dataSanPham->appends($request->all());
        } 
        else if ($request->has('chungloai')) {
            $dataSanPham = sanpham::where('id_chungloai', $request->input('chungloai'))->paginate(12);
            $dataSanPham->appends($request->all());
        } 
        else if ($request->has('SapXep')) {
            if ($request->input('SapXep') == 'new') {
                $dataSanPham = sanpham::where('isNew', 1)->paginate(12);
                $dataSanPham->appends($request->all());
            } 
            else if ($request->input('SapXep') == 'hot') {
                $dataSanPham = sanpham::where('isHot', 1)->paginate(12);
                $dataSanPham->appends($request->all());
            } 
            else {
                $dataSanPham = sanpham::orderBy('gia', $request->input('SapXep'))->paginate(12);
                $dataSanPham->appends($request->all());
            }
        } 
        else if ($request->has('TimKiem')) {
            $dataSanPham = sanpham::where('ten', 'like', '%' . $request->input('TimKiem') . '%')->paginate(12);
            $dataSanPham->appends($request->all());
        }
        else {
            $dataSanPham = sanpham::where('sale',0)->paginate(12);
